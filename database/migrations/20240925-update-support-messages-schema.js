    ...
    up: async (queryInterface, Sequelize) => {
        ...
        try {
            ...
            if (!Object.prototype.hasOwnProperty.call(describe, 'isFromAgent')) {
                    ...
                    await queryInterface.sequelize.query(
                        `UPDATE ${quotedTable}
                         SET ${quote('isFromAgent')} = CASE
                                                           WHEN ${quote('senderRole')} IS NULL THEN FALSE
                                                           WHEN LOWER(TRIM(${quote('senderRole')})) IN (${roleList}) THEN TRUE
                                                           ELSE FALSE
                             END;`,
                        { transaction }
                    );
                ...
                await queryInterface.sequelize.query(
                    `UPDATE ${quotedTable} SET ${quote('isFromAgent')} = FALSE WHERE ${quote('isFromAgent')} IS NULL;`,
                    { transaction }
                );
            }
                    ...
                    await queryInterface.sequelize.query(
                        `UPDATE ${quotedTable}
                         SET ${quote('isSystem')} = CASE
                                                        WHEN LOWER(TRIM(${quote('messageType')})) = 'system' THEN TRUE
                                                        ELSE FALSE
                             END;`,
                        { transaction }
                    );
                ...
                await queryInterface.sequelize.query(
                    `UPDATE ${quotedTable}
                     SET ${quote('senderRole')} = CASE
                                                      WHEN ${quote('isFromAgent')} = TRUE THEN 'agent'
                                                      ELSE 'client'
                         END;`,
                    { transaction }
                );
                ...
                await queryInterface.sequelize.query(
                    `UPDATE ${quotedTable}
                     SET ${quote('messageType')} = CASE
                                                       WHEN ${quote('isSystem')} = TRUE THEN 'system'
